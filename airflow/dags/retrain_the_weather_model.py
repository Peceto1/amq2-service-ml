...
markdown_text = """
### Re-Train the Model for Weather in AUS Data
...
@dag(
    dag_id="retrain_the_model_weather",
    ...
    tags=["Re-Train", "Weather"],
    default_args=default_args,
    catchup=False,
)
def processing_dag_weather():

    @task.virtualenv(
        task_id="train_the_challenger_model_weather",
    ...
    def train_the_challenger_model():
        ...
        def mlflow_track_experiment(model, X):
            ...
            mlflow.start_run(run_name='Challenger_run_' + datetime.datetime.today().strftime('%Y/%m/%d-%H:%M:%S"'),
                             ...
                             tags={"experiment": "challenger models", "dataset": "Weather AUS"},
    ...
    @task.virtualenv(
        task_id="train_the_challenger_weather_model",
        requirements=["scikit-learn==1.3.2",
                      "mlflow==2.10.2",
                      "awswrangler==3.6.0"],
        system_site_packages=True
